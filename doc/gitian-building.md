...
*Setup instructions for a Gitian build of BonaCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BonaCoin
...
- [Building BonaCoin Core](#building-bonacoin-core)
...
Clone the git repositories for bonacoin and Gitian.
...
in the bonacoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building BonaCoin Core
----------------

To build BonaCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bonacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bonacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bonacoin-project/bonacoin
...
For example:
```bash
URL=https://github.com/thrasher-/bonacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bonacoin=${COMMIT} --url bonacoin=${URL} ../bonacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bonacoin=${COMMIT} --url bonacoin=${URL} ../bonacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bonacoin=${COMMIT} --url bonacoin=${URL} ../bonacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bonacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bonacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bonacoin
SIGPATH=/some/root/path/bonacoin-detached-sigs

./bin/gbuild --url bonacoin=${BTCPATH},signature=${SIGPATH} ../bonacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bonacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bonacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bonacoin-osx-build.assert
```
...
[bonacoin-project/gitian.sigs.ltc](https://github.com/bonacoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
